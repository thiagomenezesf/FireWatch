...
import { listarLeituras } from './services/api'; // Função para buscar leituras da API
...
export default function MonitoringPageTeste() {
    ...
    try {
      ...
      setSensors((sensoresAtuais) =>
        sensoresAtuais.map((sensor) => {
          ...
          // Pega todas as leituras dessa região
          const leiturasDaRegiao = leituras.filter(
            (leitura) => leitura.regiao === sensor.name
          );

          // Pega a leitura mais recente
          const ultimaLeitura = leiturasDaRegiao.sort(
            (a, b) =>
              new Date(b.dataHora!).getTime() -
              new Date(a.dataHora!).getTime()
          )[0];

          // Se ainda não existe leitura dessa região,
          // mantém o ponto como está
          if (!ultimaLeitura) {
            return sensor;
          }
          ...
          return {
            ...sensor,
            temp: ultimaLeitura.temperatura,
            humidity: ultimaLeitura.umidade
          };
        })
      );
  ...
  { id: 1, name: 'Mirante Serra da Paulista', slug: 'mirante-serra-da-paulista', temp: null, humidity: null, status: 'atencao', x: '61.5%', y: '13%' },
  { id: 2, name: 'Região Pedra Balão', slug: 'regiao-pedra-balao', temp: null, humidity: null, status: 'normal', x: '56%', y: '81%' },
  { id: 3, name: 'Pesqueiro Bambu Amarelo', slug: 'pesqueiro-bambu-amarelo', temp: null, humidity: null, status: 'normal', x: '30.5%', y: '47%' },
  { id: 4, name: 'Capelinha Nossa Senhora', slug: 'capelinha-nossa-senhora', temp: null, humidity: null, status: 'normal', x: '40%', y: '58%' },
  { id: 5, name: 'Vinicola Lanchellotti', slug: 'vinicola-lanchellotti', temp: null, humidity: null, status: 'normal', x: '43%', y: '22%' },
  { id: 6, name: 'Cruz Cruzeiro do Sul', slug: 'cruz-cruzeiro-do-sul', temp: null, humidity: null, status: 'normal', x: '65%', y: '48%' }
]);
  ...
  const triggerEmergency = () => {
    if (simStep === 0) {
      ...
      setSensors(prev => prev.map(s => (s.id === 1 || s.id === 5) ? { ...s, temp: 41.5, status: 'critico' } : s));
      setAlerts(prev => [
      ...
      setSensors(prev => prev.map(s => (s.id === 3 || s.id === 4) ? { ...s, temp: 39.8, status: 'critico' } : s));
      setAlerts(prev => [
      ...
      setSensors([
        { id: 1, name: 'Mirante Serra da Paulista', slug: 'mirante-serra-da-paulista', temp: 31.5, humidity: 21, status: 'atencao', x: '61.5%', y: '13%' },
        { id: 2, name: 'Região Pedra Balão', slug: 'regiao-pedra-balao', temp: 29.8, humidity: 25, status: 'normal', x: '56%', y: '81%' },
        { id: 3, name: 'Pesqueiro Bambu Amarelo', slug: 'pesqueiro-bambu-amarelo', temp: 30.1, humidity: 24, status: 'normal', x: '30.5%', y: '47%' },
        { id: 4, name: 'Capelinha Nossa Senhora', slug: 'capelinha-nossa-senhora', temp: 30.8, humidity: 22, status: 'normal', x: '40%', y: '58%' },
        { id: 5, name: 'Vinicola Lanchellotti', slug: 'vinicola-lanchellotti', temp: 29.5, humidity: 21, status: 'normal', x: '43%', y: '22%' },
        { id: 6, name: 'Cruz Cruzeiro do Sul', slug: 'cruz-cruzeiro-do-sul', temp: 32.1, humidity: 23, status: 'normal', x: '65%', y: '48%' },
      ]);
              ...
              <div className="map-canvas" style={{
                ...
                {sensors.map(s => (
                  <div key={s.id} style={{
                    ...
                    {/* HALO / HEATMAP */}
                    <div
                      className={`sensor-heat sensor-heat-${s.status}`}
                    />

                    {/* PONTO DO SENSOR */}
                    <div
                      className={`sensor-dot sensor-dot-${s.status}`}
                      onClick={() => navigate(`/monitoramento/regiao/${s.slug}`)}
                    />
                    <div style={{
                      ...
                      <strong>{s.name}</strong>{' | '}{s.temp !== null ? `${s.temp}°C` : 'Sem leitura'}{s.humidity !== null && ` | ${s.humidity}%`}
                    </div>
                  </div>
                ))}
